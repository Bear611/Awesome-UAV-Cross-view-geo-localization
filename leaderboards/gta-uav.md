...
UAV geo-localization benchmark from game data (Dai et al. 2024, Game4Loc). Two evaluation settings: Same-Area (query and gallery from same area) and Cross-Area (true generalization test). Main results use Positive+Semi-positive training data. Metrics: R@1/R@5/AP (retrieval), SDM@3 / Dis@1 (localization, in meters). Rows sorted by R@1 descending.
...
| Weighted-InfoNCE + MES (Pos-only) | 75.97 | 94.53 | 83.35 | 82.80 | 325.61 | Game4Loc: A UAV Geo-Localization Benchmark from Game Data (Dai et al. 2024 — GTA-UAV paper) | Table 2 (Positive-only block, Ours row, Same-Area column), p.5 |
| InfoNCE Loss (Pos-only) | 72.99 | 90.64 | 80.76 | 80.40 | 363.67 | Game4Loc: A UAV Geo-Localization Benchmark from Game Data (Dai et al. 2024 — GTA-UAV paper) | Table 2 (Positive-only block, InfoNCE row, Same-Area column), p.5 |
...
| Pretrain on ImageNet | 10.65 | 23.90 | 17.15 | 36.82 | 1470.50 | Game4Loc: A UAV Geo-Localization Benchmark from Game Data (Dai et al. 2024 — GTA-UAV paper) | Table 3 (ImageNet row), p.6 |
...
| Method | R@1 | R@5 | AP | SDM@3 | Dis@1 | Paper | Source |
| --- | ---: | ---: | ---: | ---: | ---: | --- | --- |
| Weighted-InfoNCE + MES (Pos-only) | 57.52 | 80.10 | 67.24 | 72.33 | 444.13 | Game4Loc: A UAV Geo-Localization Benchmark from Game Data (Dai et al. 2024 — GTA-UAV paper) | Table 2 (Positive-only block, Ours row, Cross-Area column), p.5 |
...
| Swinv2-B (backbone) | 53.70 | - | - | - | - | Game4Loc: A UAV Geo-Localization Benchmark from Game Data (Dai et al. 2024 — GTA-UAV paper) | Table 5 (Swinv2-B row), p.6 |
| InfoNCE + MES (Pos-only) | 52.64 | 74.63 | 62.40 | 67.64 | 552.90 | Game4Loc: A UAV Geo-Localization Benchmark from Game Data (Dai et al. 2024 — GTA-UAV paper) | Table 2 (Positive-only block, InfoNCE+MES row, Cross-Area column), p.5 |
...
| Triplet Loss (Pos-only) | 43.41 | 66.70 | 53.56 | 61.26 | 756.95 | Game4Loc: A UAV Geo-Localization Benchmark from Game Data (Dai et al. 2024 — GTA-UAV paper) | Table 2 (Positive-only block, TripletLoss row, Cross-Area column), p.5 |
...
| Pretrain on ImageNet | 9.74 | 21.73 | 15.74 | 33.58 | 1841.30 | Game4Loc: A UAV Geo-Localization Benchmark from Game Data (Dai et al. 2024 — GTA-UAV paper) | Table 3 (ImageNet row), p.6 |
